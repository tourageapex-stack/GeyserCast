...
import { Clock, MapPin, Navigation, Heart, Bell, ChevronRight, ShieldCheck, Flame, Car, Compass, ExternalLink, Map } from 'lucide-react';
...
interface PredictionCardProps {
  ...
  onShowOnMap: (geyserId: string) => void;
  onPlanVisit: (geyserId: string) => void;
  userLat: number;
  userLon: number;
}
...
export const PredictionCard: React.FC<PredictionCardProps> = ({
  ...
  const directionsUrl = `https://www.google.com/maps/dir/?api=1&origin=${userLat},${userLon}&destination=${geyser.latitude},${geyser.longitude}&travelmode=walking`;
    ...
    <div className="bg-stone-900 border border-stone-800 rounded-2xl p-5 shadow-lg hover:border-amber-500/40 transition flex flex-col justify-between space-y-4 text-stone-100">
      ...
      <div className="bg-stone-950/60 border border-stone-800 rounded-xl p-3.5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          <div className="flex items-center space-x-2 bg-stone-900/80 p-2 rounded-lg border border-stone-800">
            ...
            <div>
              ...
              <div className="text-stone-200 font-bold text-sm">
                Walk {formatMinutesToHoursAndMinutes(walkRoute.durationMinutes)} ({walkRoute.distanceMiles} mi)
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 bg-stone-900/80 p-2 rounded-lg border border-stone-800">
            ...
            <div>
              ...
              <div className="text-stone-200 font-bold text-sm">
                Drive {formatMinutesToHoursAndMinutes(driveRoute.durationMinutes)} ({driveRoute.distanceMiles} mi)
              </div>
            </div>
          </div>
        </div>
        ...
        <div className={`p-2.5 rounded-lg border flex items-center justify-between text-xs font-bold ${makeItBadgeStyle}`}>
          ...
          <span className="text-[11px] opacity-80">
            Margin:{' '}
            {canMakeIt.marginMinutes >= 0
              ? `+${formatMinutesToHoursAndMinutes(canMakeIt.marginMinutes)}`
              : `-${formatMinutesToHoursAndMinutes(Math.abs(canMakeIt.marginMinutes))}`}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-2 pt-1">
        <div className="flex items-center space-x-1 text-xs text-stone-400">
          ...
          <span>
            Last eruption: {formatMinutesToHoursAndMinutes(Math.abs(prediction.features.currentIntervalMinutes))} ago
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] font-bold">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center justify-center space-x-1.5 py-2 px-2 rounded-lg border border-stone-700 bg-stone-800 hover:bg-stone-700 text-sky-300"
          >
            <ExternalLink className="w-3.5 h-3.5" />
            <span>Directions</span>
          </a>
          <button
            onClick={() => onShowOnMap(geyser.id)}
            className="flex items-center justify-center space-x-1.5 py-2 px-2 rounded-lg border border-stone-700 bg-stone-800 hover:bg-stone-700 text-emerald-300"
          >
            <Map className="w-3.5 h-3.5" />
            <span>Show on Map</span>
          </button>
          <button
            onClick={() => onPlanVisit(geyser.id)}
            className="flex items-center justify-center space-x-1.5 py-2 px-2 rounded-lg border border-stone-700 bg-stone-800 hover:bg-stone-700 text-amber-300"
          >
          ...
          <button
            onClick={() => onSelectGeyser(geyser.id)}
            className="flex items-center justify-center space-x-1.5 py-2 px-2 rounded-lg border border-amber-700 bg-amber-950/60 hover:bg-amber-900/70 text-amber-300"
          >
            <span>Details & Charts</span>
